...
def Input():
    x, y = [], []
    with open('inputNewtonBatKy.txt', 'r+') as f:  # đọc file input
        for line in f.readlines():  # duyệt từng hàng trong input và thêm vào mảng x y
...
def BuildBTH(BTH, x, y, i):
    TH_row_i = np.zeros(len(x))  ### ban đầu đã khởi tạo ma trận 1 chiều BTH[0]=y[0]
    TH_row_i[0] = y[i]
...
def BuildBTT(BTT, x, i):
    TT_row_i = np.zeros(len(x))  ### ban đầu đã khởi tảo ma trận 1 chiều BTT[0]=1
...
# ước tính f(x0)
def fx0(f, n, x0):
    value = f[n]  # n là bậc, valua là hệ số của bậc cao nhất
...
def Newton_interpolation_Backward(BTH, BTT, f, x, y, n):
    x.reverse()  # đảo ngược lại mảng x
    y.reverse()
    BTH[0], f[0] = y[0], y[0]
...
def add_Newton(BTH, BTT, f, x, y, n):
    ...
    f = np.hstack((f, np.zeros(1)))  # tạo thêm 1 cột cho f để điền hệ số x thêm vào
    f = fx(BTH, BTT, n, f)
    return f, BTH, BTT
...
def main():
    ...
    value = fx0(f, n, x0)
    print("Hệ số đa thức nội suy Newton là: ")
    print(f)
